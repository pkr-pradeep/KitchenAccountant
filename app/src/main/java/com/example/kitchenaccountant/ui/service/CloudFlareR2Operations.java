...
public class CloudFlareR2Operations {
    public static void saveObjectIntoR2UsingCompletableFuture(String transactionType, String formDataString) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            CompletableFuture.runAsync(() -> {
                try {
                    PutObjectIntoCloudFlare.putS3Object(transactionType, formDataString, ISKCON_KITCHEN_MGT_BUCKET, AWS_REGION, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY);
                } catch (Exception e) {
